...
<xsl:param name="xsltInfo"/>
<xsl:param name="authors" />
/*  © Copyright CERN<xsl:if test="$xmlRoot/d:design/@vendor">, <xsl:value-of select="$xmlRoot/d:design/@vendor"/></xsl:if>, 2015. All rights not expressly granted are reserved.
    Authors(from Quasar team): <xsl:value-of select="$authors"/>
    ...
    <xsl:if test="$xmlRoot/d:design/@vendor">The stub of this file was made complete by <xsl:value-of select="$xmlRoot/d:design/@vendor"/></xsl:if>
